...
#pragma warning disable 612, 649
#pragma warning disable 168

namespace Fusion.GIS.DataSystem.GeoObjectsSources
{
	public class OpenStreetMapSource
	{
		public Game     Game;
		public MapLayer Map;
		...
		public Dictionary<long, osmNode>		allNodes;
		public Dictionary<long, osmWay>			allWays;
		public Dictionary<long, osmRelation>	allRelations;
		...
		public OpenStreetMapSource(Game game, MapLayer mapLayer)
		{
			...
			//DrawShader		= rs.CreateUberShader("GeoObjects.hlsl", typeof(DrawFlags));
			//DrawParameters	= rs.CreateConstBuffer<DrawParams>();
			DrawShader		= Game.Content.Load<Ubershader>( @"GeoObjects.hlsl" );
			DrawShader.Map(typeof(DrawFlags) );
			DrawParameters	= new ConstantBuffer( rs, typeof(DrawParams) );
		...
		//	vertexBuf = rs.CreateVertexBuffer(typeof(Vertex), verts.Length);
		...
		public void Update( GameTime gameTime, StereoEye stereoEye )
		{
			var rs = Game.GraphicsDevice;
			...
			string sig;
			DrawShader.SetVertexShader((int)DrawFlags.NONE);
			DrawShader.SetPixelShader((int)DrawFlags.NONE);

			var constDrawParams				= new DrawParams();

			constDrawParams.Zoom			= new Vector4(Map.Zoom);
			constDrawParams.Offset			= new Vector4(Map.Offset, 0.0f, 0.0f);
			//DrawParameters.Data.viewProjMatrix = cam.ProjOrthoMatrix;
			constDrawParams.viewProjMatrix	= cam.GetViewMatrix( stereoEye )*cam.GetProjectionMatrix( stereoEye );
			DrawParameters.SetData(constDrawParams);

			rs.VertexShaderConstants[0]	= DrawParameters;

			var VertInLayout	= new VertexInputLayout( rs, typeof(Vertex) );
			rs.SetupVertexInput( VertInLayout, vertexBuf, null );


			rs.BlendState			=	BlendState.AlphaBlend;
