...
namespace Fusion.GIS.DataSystem.MapSources.YandexMaps
{
	...
	public class YandexMap : BaseYandexMap
	{
		...
		{
			return String.Format(UrlFormat, UrlServer, GetServerNum(x, y, 4) + 1, x, y, zoom);
		}


		static readonly string UrlServer = "vec";
		static readonly string UrlFormat = "http://{0}0{1}.maps.yandex.net/tiles?l=map&x={2}&y={3}&z={4}";
		static readonly string UrlFormatOld = "http://{0}0{1}.maps.yandex.net/tiles?l=map&v={2}&x={3}&y={4}&z={5}";
	}
