...
namespace Fusion.GIS.LayerSpace.Layers
{
	public partial class MapLayer : BaseLayer
	{
		...
		public BaseMapSource OSMMapSource;
	...
	//		OSMMapSource	= new OpenStreetMapNodes(game);
		...
		public void Draw2( GameTime gameTime, Fusion.Graphics.StereoEye stereoEye )
		{
			var rs = Game.GraphicsDevice;
			var cam = Game.GetService<Camera>();
			//var dr = rs.DebugRender;
			...
			sb.Begin( 
						...
						RasterizerState.CullCCW,
						DepthStencilState.Default,
						cam.GetViewMatrix( stereoEye )*cam.GetProjectionMatrix( stereoEye )
					);
...
//			vertexBuffer = Game.GraphicsDevice.CreateVertexBuffer(typeof(Vertex), MaxVertices);
			vertexBuffer	= new VertexBuffer( Game.GraphicsDevice, typeof(Vertex), MaxVertices );


	//		constBuffer = Game.GraphicsDevice.CreateConstBuffer<ConstData>();
			constBuffer	= new ConstantBuffer( Game.GraphicsDevice, typeof(ConstData) );
	//		shader		= Game.GraphicsDevice.CreateUberShader(@"Core\Map.hlsl", typeof(MapFlags));
	//		shader		= Game.Content.Load<Ubershader>(@"Core\Globe.hlsl");
			shader		= Game.Content.Load<Ubershader>(@"Map.hlsl");
			shader.Map( typeof(MapFlags) );
		}
		...
		public void Draw3(GameTime gameTime)
		{
			...
			StereoEye stereoEye	= new StereoEye();

			cbData.ViewProj		= cam.GetViewMatrix( stereoEye ) * cam.GetProjectionMatrix( stereoEye );
			cbData.Zoom			= new Vector4(Zoom);
			cbData.Offset		= new Vector4(Offset, 0.0f, 0.0f);
			cbData.Padding		= new Vector4();
			cbData.TesAmount	= new Vector4(TesselationFactor);
			...
			dev.DomainShaderSamplers[0]	= SamplerState.LinearClamp;

			var VertInLayout	= new VertexInputLayout( dev, typeof(Vertex) );
			dev.SetupVertexInput( VertInLayout, vertexBuffer, null);

			dev.BlendState			= BlendState.Opaque;
			dev.DepthStencilState	= DepthStencilState.Default;

			dev.RasterizerState	= Game.InputDevice.IsKeyDown(Keys.NumPad1) ? RasterizerState.Wireframe : RasterizerState.CullCW;

			for (int i = 0; i < srvs.Count; i++) {
	//			srvs[i].SetPS(0);
				dev.PixelShaderResources[0]	= srvs[i]; // ??? maybe

	//			quads[i].SRV0.SetDS(1);
				dev.DomainShaderResources[1]	= quads[i].SRV0; // ??? maybe

	//			quads[i].SRV1.SetDS(2);
				dev.DomainShaderResources[2]	= quads[i].SRV1; // ??? maybe

	//			quads[i].SRV2.SetDS(3);
				dev.DomainShaderResources[3]	= quads[i].SRV2; // ??? maybe

	//			quads[i].SRV3.SetDS(4);
				dev.DomainShaderResources[4]	= quads[i].SRV3; // ??? maybe

	//			quads[i].SRV0.SetPS(1);
				dev.PixelShaderResources[1]	= quads[i].SRV0; // ??? maybe

	//			quads[i].SRV1.SetPS(2);
				dev.PixelShaderResources[2]	= quads[i].SRV1; // ??? maybe

	//			quads[i].SRV2.SetPS(3);
				dev.PixelShaderResources[3]	= quads[i].SRV2; // ??? maybe

	//			quads[i].SRV3.SetPS(4);
				dev.PixelShaderResources[4]	= quads[i].SRV3; // ??? maybe


				rs.Draw(Primitive.PatchList4CP, 4, 4 * i);
