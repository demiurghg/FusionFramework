...
namespace Fusion.GIS.LayerSpace.Layers
{
	public partial class GlobeLayer
	{
		...
		VertexBuffer		dotsVB;
		ConstantBuffer		dotsBuf;
		DotsConstData		dotsConstData;
		...
		void InitDots()
		{
			...
			dotsConstData = new DotsConstData();
			dotsConstData.Colors = new float[16*4];

			dotsBuf = new ConstantBuffer( Game.GraphicsDevice, typeof( DotsConstData ) );

			//var r = new Random();
			
			Dots = new GeoVert[500100];
			
	//		dotsVB = Game.GraphicsDevice.CreateVertexBuffer(typeof(GeoVert), Dots.Length);
		...
		//	simpleRailroadsVB = Game.GraphicsDevice.CreateVertexBuffer(typeof(GeoVert), 18);
			...
			dotsConstData.Colors[0] = 1.0f;
			dotsConstData.Colors[1] = 1.0f;
			dotsConstData.Colors[2] = 1.0f;
			dotsConstData.Colors[3] = 1.0f;
			...
			socioClasses	= Game.Content.Load<Texture2D>("Urban/SocioClasses.png");
			geoObjects		= Game.Content.Load<Texture2D>("Urban/geoObjects.png");
			infectionDot	= Game.Content.Load<Texture2D>("Urban/InfectionDot.png");
		}
		...
		public void UpdateColors(Color[] cols)
		{
			...
			for (int i = 1; i < count+1; i++) {
				...
				dotsConstData.Colors[i * 4 + 0] = v.X;
				dotsConstData.Colors[i * 4 + 1] = v.Y;
				dotsConstData.Colors[i * 4 + 2] = v.Z;
				dotsConstData.Colors[i * 4 + 3] = v.W;
