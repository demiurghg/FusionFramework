	...
	public partial class GlobeLayer
	{
		...
		void AddTileToRenderList(int x, int y, int zoom)
		{
			...
			if (tilesFree.Any()) {
				...
				GenerateTileGrid(tileDensity, out tile.VertexBuf, out tile.IndexBuf, x0, x1, y0, y1);
		...
		void GenerateTileGrid(int density, out VertexBuffer vb, out IndexBuffer ib, double left, double right, double top, double bottom)
		{
			int[]	indexes; 
			GeoVert[]	vertices;
		...
		void CalculateVertices(out GeoVert[] vertices, out int[] indeces, int density, double left, double right, double top, double bottom)
		{
			...
			for (int row = 0; row < RowsCount; row++) {
				for (int col = 0; col < ColumnsCount; col++) {
					...
					float elev = 0.0f;
					//elev = el.GetElevation(lon, lat) / 1000.0f;
		...
		void DetermineTiles()
		{
			//SetCurrentLevel();
			var ms = Game.GetService<LayerService>().MapLayer.CurrentMapSource;
			var osmsrc	= Game.GetService<LayerService>().MapLayer.OSMMapSource;
